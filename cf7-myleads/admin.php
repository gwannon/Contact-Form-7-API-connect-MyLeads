...
function my_leads_plugin_menu() {
	add_options_page( __('Administración MyLeads'), __('MyLeads'), 'manage_options', 'my_leads', 'my_leads_page_settings');
}

function my_leads_page_settings() { 
	//echo "<pre>"; print_r($_REQUEST); echo "</pre>";
	if(isset($_REQUEST['send']) && $_REQUEST['send'] != '') { 
		?><p style="border: 1px solid green; color: green; text-align: center;"><?php _e("Datos guardados correctamente."); ?></p><?php
	...
	<h2><?php _e('Campos obligatorios que deben tener esos formularios.'); ?></h2>
	<ul>
		<li>[select* myleads-salutation "Señor|MR" "Señora|MRS"]</li>
		<li>[text* myleads-firstname]</li>
		<li>[text* myleads-lastname-1]</li>
		<li>[text* myleads-mobile-phone<]</li>
		<li>[text* myleads-email]</li>
	</ul>
	<h2><?php _e('Campos opcionales que deben tener esos formularios.'); ?></h2>
	<ul>
	...
	<p><small>* También se puede meter en la zona de administración de MyLeads.</small></p> 
	<?php
}
